defmodule StarterKit.Accounts.User.Senders.SendNewUserConfirmationEmail do
  @moduledoc """
  ...
  defp body(token, opts) do
    # KTD3 scheme: confirmation callback is a plain GET at /auth/confirm?token=... (U5).
    url =
      StarterKitWeb.Endpoint.url() <>
        "/auth/confirm?" <> URI.encode_query(token: token)
